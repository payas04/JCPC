...
const Login = ({ email, setEmail, password, setPassword, handleLogin }) => {
  ...
  useEffect(() => {
    ...
    .to([logoRef.current, cardRef.current], {
      x: (index) => ["-=170", "+=150"][index],
      opacity: (index) => index === 1 ? 1 : 1,
      duration: 0.6,
      ease: "power2.out"
    }, "+=0.2")
    .to([leftGridRef.current, rightGridRef.current], {
      opacity: 1,
      duration: 0.5,
      ease: "power2.out",
    }, "+=0.2");
  ...
  return (
    <div className="h-screen w-full bg-white overflow-hidden relative">
      {/* Left Grid */}
      <div ref={leftGridRef} className="absolute left-12 top-1/2 -translate-y-1/2">
        <Grid />
      </div>
      ...
      {/* Right Grid */}
      <div ref={rightGridRef} className="absolute right-12 top-1/2 -translate-y-1/2">
        <Grid />
      </div>
    </div>
  );
